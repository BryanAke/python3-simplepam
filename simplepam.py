...
def authenticate_complex(username, responses, service='login', encoding='utf-8',
                 resetcred=True):
    """Returns True if the given username and password authenticate for the
    given service.  Returns False otherwise.
    ...
    ``responses``: dictionary with responses for prompts.
                    e.g. {
                        'Password:': 'hunter2',
    ...
    if sys.version_info >= (3,):
        ...
        for key, val in responses.items():
            if isinstance(val, str):
                responses[key] = val.encode(encoding)
        if isinstance(service, str):
            service = service.encode(encoding)
    ...
    def my_conv(n_messages, messages, p_response, app_data):
        ...
        for i in range(n_messages):
            if messages[i].contents.msg_style == PAM_PROMPT_ECHO_OFF:
                print(messages[i].contents.msg)
                pw_copy = strdup(responses[messages[i].contents.msg])
                p_response.contents[i].resp = cast(pw_copy, c_char_p)
                p_response.contents[i].resp_retcode = 0
        return 0
